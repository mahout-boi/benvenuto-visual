...
const Contact = () => {
  ...
  return (
    <section id="contato" className="bg-secondary/40 py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div
          ref={content.ref}
          className={`mt-14 grid gap-10 md:grid-cols-2 items-start transition-all duration-700 delay-150 ease-out ${content.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
          {/* Info column */}
          <div className="space-y-8">
            ...
            <div className="flex items-start gap-5">
              <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-border bg-background shadow-sm">
            ...
            <div className="flex items-start gap-5">
              ...
              <div className="w-full">
                ...
                <div
                  className={`overflow-hidden transition-all duration-500 ease-in-out ${open ? "max-h-[500px] opacity-100 mt-4" : "max-h-0 opacity-0"}`}
                >
                  <div className="space-y-2">
                    {horarios.map((item) => (
                      ...
                      >
                        <p className="text-xs font-medium">{item.dia}</p>
                        <div className="flex gap-2 flex-wrap justify-end">
                          {item.periodos.map((periodo, i) => (
                            <span
                              key={i}
                              className="rounded-full bg-secondary px-3 py-1 text-[10px] tracking-wide text-muted-foreground"
                            >
                              {periodo}
                            </span>
                          ))}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-full bg-foreground px-8 py-3.5 text-[10px] uppercase tracking-[0.25em] text-background transition-opacity hover:opacity-80"
            >
          ...
          {/* Mapa */}
          <div className="h-[400px] overflow-hidden rounded-sm border border-border shadow-sm">
            <iframe
