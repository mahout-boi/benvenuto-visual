...
import { horarios, WHATSAPP_URL } from "@/lib/constants";
    ...
    <section id="contato" className="bg-secondary/40 py-16 sm:py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="mt-3 sm:mt-4 font-playfair-display text-3xl sm:text-4xl md:text-5xl font-light">Visite-nos</h2>
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="flex items-start gap-4 sm:gap-5">
              ...
              <div>
                ...
                <p className="text-sm leading-relaxed text-foreground">Rua Dante Grossi, 249<br />Garibaldi — RS</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 sm:gap-5">
              ...
              <div className="w-full min-w-0">
                <button onClick={() => setOpen(!open)} className="flex w-full items-center justify-between">
                  ...
                  <ChevronDown size={15} className={`transition-transform duration-300 text-muted-foreground ${open ? "rotate-180" : ""}`} />
                </button>
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${open ? "max-h-[500px] opacity-100 mt-3 sm:mt-4" : "max-h-0 opacity-0"}`}>
                  <div className="space-y-1.5 sm:space-y-2">
                    {horarios.map((item) => (
                      <div key={item.dia} className="flex items-center justify-between rounded-sm border border-border bg-background px-3 sm:px-4 py-2.5 sm:py-3 gap-2">
                        <p className="text-[11px] sm:text-xs font-medium shrink-0">{item.dia}</p>
                        <div className="flex gap-1.5 sm:gap-2 flex-wrap justify-end">
                          {item.periodos.map((periodo, i) => (
                            <span key={i} className="rounded-full bg-secondary px-2 sm:px-3 py-0.5 sm:py-1 text-[9px] sm:text-[10px] tracking-wide text-muted-foreground whitespace-nowrap">{periodo}</span>
                          ))}
                        </div>
                      </div>
                    ))}
                ...
                {!open && <p className="mt-1 text-sm text-foreground">Seg–Dom &nbsp;·&nbsp; Almoço e Jantar</p>}
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 sm:gap-5">
              ...
              <div>
                ...
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="text-sm text-foreground underline-offset-4 hover:underline transition-colors hover:text-accent">(54) 9674-3601</a>
              </div>
            </div>

            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-foreground px-6 sm:px-8 py-3 sm:py-3.5 text-[10px] uppercase tracking-[0.25em] text-background transition-opacity hover:opacity-80">
